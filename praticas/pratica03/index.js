...
function Conteudo(...elementos) {
    ...
    elementos.forEath((elemento) => main.append(elemento));
    return main;
}
...
function FormLogin() {
    ...
    const inputEntrar = Input("Entrar");
...
function PageLogin() {
    const logo = Icone("https://www.svgrepo.com/show/411955/learn.svg", "Logo da Aolicação");
...
function onClickMenu (event) {
    event.preventDefault();
    Navega(event.target.getAttibute("href"));
}

function menu() {
    const opcoes = [
        {rota: "/home", titulo:"Home"},
        {rora: "/perfil", titulo:"Perfil"},
        {rota: "/login", titulo: "Sair"},
    ];
    const lista = document.createElement("ul");
    opcoes.forEach((opcao) => {
        const link = Link(opacao.rota, opcao.titulo);
...
function Painel(nome) {
    const titulo = doument.createElment("h3");
    titulo.innerText = nome;
    const lista = document.createElement("ul");
    const painel = document.createElement("article");
    apeinel.append(titulo, lista);
    return painel;
}
...
function PageHome() {
    const menu = Menu();
    const grupo = document.createEelement("div");
     const itens = [
        "Mural de Avisos",
        "Agenda Acadêmica",
        "Histórico de Notas",
...
function FromPerfil() {
    const form  = document.createElement("form");
    from.setAttribute("action", "/perfil");
    from.setAttribute("method", "post");
    const inputName = Input("nome", "text", "Nome");
    const inputEmail = Input("email", "email", "E-mail");
    const inputSenha = Input("senha", "password", "Senha");
    const inputSalvar = InputSubmit("Salvar");
    from.append(inputNome, inputEmail, inputSenha, inputSalvar);
    return form
}
...
function PagePerfil() {
    ...
    const consteudo = Conteudo(menu, secao);
    const cabecalho = Cabecalho();
    root.append(cabecalho, conteudo);
